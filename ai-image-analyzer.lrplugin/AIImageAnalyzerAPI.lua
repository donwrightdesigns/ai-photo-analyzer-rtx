...
-- Configuration
local API_BASE_URL = "http://localhost:5001"
local TIMEOUT = 120
...
local function getPreferences()
    ...
    return {
        ...
        modelType = prefs.modelType or "gemini",
        promptProfile = prefs.promptProfile or "professional_art_critic",
...
local function checkServiceStatus()
    ...
    if headers and headers.status == 200 then
        ...
        -- Service is available if we can connect, regardless of specific model availability
        -- Individual models will be checked when processing starts
        return true
    end
...
-- Function to set prompt profile on server
local function setPromptProfile(prefs)
    local postData = JSON:encode({
        profile_key = prefs.promptProfile
    })
    
    LrHttp.post(
        prefs.apiUrl .. "/api/set_prompt_profile", 
        postData, 
        {
...
local function analyzeImageViaAPI(imagePath, prefs)
    -- Set the prompt profile first
    setPromptProfile(prefs)
